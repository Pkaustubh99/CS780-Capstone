...
# Save / Load
# -----------------------
def saveModel(model, path="algo52/weights.pth"):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    torch.save(model.state_dict(), path)


def loadModel(model, path="algo52/weights.pth"):
...
base_path = "algo52/base_path.csv"
edited_path = "algo52/edited_path.csv"
...
def train(env, episodes=1000):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            epsilon = max(0, 0.6 - (0.5 * (ep/1000))) # 20% random
