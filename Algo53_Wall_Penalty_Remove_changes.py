...
# Save / Load
# -----------------------
def saveModel(model, path="algo53/weights.pth"):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    torch.save(model.state_dict(), path)


def loadModel(model, path="algo53/weights.pth"):
...
base_path = "algo53/base_path.csv"
edited_path = "algo53/edited_path.csv"
...
def train(env, episodes=10000):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            epsilon = max(0, 0.4 - (0.4 * (ep/10000))) # 20% random
            ...
            nextState, reward, done = env.step(ACTIONS[actionIndex],render= False)
